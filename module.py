...
class TGAN(torch.nn.Module):
    ...
    def tem_conv(self, src_idx_l, cut_time_l, curr_layers, n_ngh=20):
        ...
        if not self.training and self._opt.enabled:
            return self._opt_tem_conv(src_idx_l, cut_time_l, curr_layers,
                                      n_ngh)
        else:
            return self._base_tem_conv(src_idx_l, cut_time_l, curr_layers,
                                       n_ngh)

    def _base_tem_conv(self, src_idx_l, cut_time_l, curr_layers, n_ngh=20):
        if curr_layers == 0:
            src_node_batch_th = torch.from_numpy(src_idx_l).long().to(
                self.device)
            ...
            src_ngh_node_batch, src_ngh_eidx_batch, src_ngh_t_batch = self.ngh_finder.ngh_lookup(
                src_idx_l, cut_time_l, n_ngh=n_ngh)

            src_ngh_node_batch_th = torch.from_numpy(
                src_ngh_node_batch).long().to(self.device)
            src_ngh_eidx_batch = torch.from_numpy(
                src_ngh_eidx_batch).long().to(self.device)

            #torch.cuda.synchronize()
            t_start = time.perf_counter()
            src_ngh_t_batch_delta = cut_time_l[:, np.newaxis] - src_ngh_t_batch
            src_ngh_t_batch_th = torch.from_numpy(
                src_ngh_t_batch_delta).float().to(self.device)
            #torch.cuda.synchronize()
            ...
            src_ngh_node_conv_feat = self._base_tem_conv(
            ...
            #torch.cuda.synchronize()
            t_start = time.perf_counter()
            cut_time_l_th = torch.from_numpy(cut_time_l).float().to(
                self.device)
            cut_time_l_th = torch.unsqueeze(cut_time_l_th, dim=1)
            src_node_t_embed = self.time_encoder(
                torch.zeros_like(cut_time_l_th))
            #torch.cuda.synchronize()
            ...
            #torch.cuda.synchronize()
            t_start = time.perf_counter()
            local = attn_m(src_node_conv_feat, src_node_t_embed, src_ngh_feat,
                           src_ngh_t_embed, src_ngh_edge_feat, mask)
            #torch.cuda.synchronize()
    ...
    def _compute_embed(self, src_l: np.ndarray, ts_l: np.ndarray, layer: int,
                       n_ngh: int):
        if layer == 0:
            ...
            src_embed = self._compute_embed(src_l,
                                            ts_l,
                                            layer=layer - 1,
                                            n_ngh=n_ngh)
            ...
            ngh_embed = self._opt_tem_conv(ngh_batch.flatten(),
                                           ngh_ts_batch.flatten(),
                                           layer=layer - 1,
                                           n_ngh=n_ngh)
            ...
            ngh_ts_delta = torch.from_numpy(ngh_ts_delta).float().to(
                self.device)
            #torch.cuda.synchronize()
            self._opt._t_time_encode_nghs += (time.perf_counter() - t_start)
            ...
            if self._opt.enabled_time:
                ...
                ngh_t_embed = self._opt.compute_time_embed(
                    ngh_ts_delta, self.time_encoder)
            else:
                ...
                #torch.cuda.synchronize()
                self._opt._t_time_encode_zero += (time.perf_counter() -
                                                  t_start)

                t_start = time.perf_counter()
                ngh_t_embed = self.time_encoder(ngh_ts_delta)
                #torch.cuda.synchronize()
                self._opt._t_time_encode_nghs += (time.perf_counter() -
                                                  t_start)

            ngh_eidx_batch = torch.from_numpy(ngh_eidx_batch).long().to(
                self.device)
            ...
            #torch.cuda.synchronize()
            t_start = time.perf_counter()
            local_embed = attn_m(src_embed, src_t_embed, ngh_embed,
                                 ngh_t_embed, ngh_edge_feat, mask)
            #torch.cuda.synchronize()
            self._opt._t_attn += (time.perf_counter() - t_start)
