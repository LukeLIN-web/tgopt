...
parser.add_argument('--model',
                    type=str,
                    required=True,
                    help='prefix for loading saved model')
parser.add_argument('--prefix',
                    type=str,
                    required=True,
                    help='prefix for this inference run')
...
parser.add_argument('--save-embeds',
                    action='store_true',
                    help='save embeddings for each batch')
...
parser.add_argument('--csv',
                    type=str,
                    default='',
                    help="csv file to write avg/std into")
...
for r in range(args.runs):
    ...
    for k in range(num_batch):
        ...
        with torch.no_grad():
            ...
            embed = model.tem_conv(node_l_cut,
                                   time_l_cut,
                                   NUM_LAYER,
                                   n_ngh=NUM_NEIGHBORS)
            #torch.cuda.synchronize()
    ...
    if args.stats:
        ...
        if opt.enabled_cache:
            ...
            batch_df.to_csv(f'logs/{args.prefix}-{args.data}-hits.csv',
                            index=False)
